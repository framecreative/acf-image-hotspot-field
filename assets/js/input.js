(function( $ ) {
    ...
    // the 'source type' to determine where we grab the image'
    sourceType = $fieldImg.attr('data-source-type'),
		...
		handleClick = function( e ) {
			...
			if ( percentageBased ) {
				x = ( parseInt( x ) / imgWidth * 100 ).toFixed( 2 ) + '%';
				y = ( parseInt( y ) / imgHeight * 100 ).toFixed( 2 ) + '%';
			}
		...
		setLinkedImg = function() {
		  ...
		  if ( sourceType === 'function' ){
		    return setLinkedImageFromFunction();
      }

		  if ( sourceType === 'attachment' ){
		    return setLinkedImageFromAttachment();
      }
      return setLinkedImageFromField();
		},

    setLinkedImageFromField = function(){
    ...
    setLinkedImageFromFunction = function(){

		  var url = $fieldImg.attr('data-url');
